...
async function passwortReset() {
  try {
    ...
    if (error.code === "auth/user-not-found") {
      ...
      errorMsg.value =
        "Das hat leider nicht geklappt. Bitte versuche es später erneut oder wende dich an unseren Support.";
    }
  }
}
...
<template>
  ...
  <div class="md:mx-auto md:w-full md:max-w-md">
    <h2 class="mt-6 text-center text-2xl font-bold leading-9 text-gray-900">
      Nutzer anpassen
    </h2>
  </div>

  <!-- Disclaimer -->
  <div v-if="provider == 'google.com'" class="mb-5">
    <div class="mt-10 md:mx-auto md:w-full md:max-w-[800px]">
      ...
      >
        ...
        <p class="mt-6 leading-8">
          Vielen Dank, dass Sie sich über Google angemeldet haben. Ihre
          Nutzerdaten werden automatisch von Google übernommen und können nicht
          geändert werden.
        </p>
      </div>
    </div>
  ...
  <div v-else class="mb-5">
    <div class="mt-10 md:mx-auto md:w-full md:max-w-[800px]">
      ...
      >
        ...
        <p class="mt-6 leading-8">
          Ihr Account besitzt noch keine Verbindung zu unserem SSO System.
          Klicken Sie auf jetzt verbinden und nutzen Sie zukünftig direkt den
          Google SSO Login ganz ohne Eingabe von Emailadresse und Passwort!
        </p>
        <button
          @click="linkToSso"
          class="flex w-full justify-center rounded-md px-3 p-1.5 text-sm font-semibold leading-6 text-ms-white bg-primary mt-3"
        >
          SSO jetzt verbinden
        </button>
      </div>
    ...
    <div class="bg-ms-white px-6 py-12 shadow md:rounded-lg md:px-12">
      ...
      <div class="">
        ...
        <p class="mb-2 text-gray-900">
          Passen Sie ihre Emailadresse sowie Ihren Benutzernamen an.
        </p>
        <form class="space-y-6" @submit.prevent="updateUser">
          <div>
            ...
            <div class="mt-2">
              <input
                :disabled="provider == 'google.com'"
                id="email"
                v-model="email"
                name="email"
                type="email"
                autocomplete="email"
                class="block w-full shadow-sm ring-1 ring-inset ring-gray-300 rounded-md border-0 text-gray-900 placeholder:text-gray-400 focus:ring-2 focus:ring-primary"
                :class="provider == 'google.com' ? 'opacity-30' : ''"
              />
            </div>
          </div>
          <p
            v-if="errors.emailUpdate"
          ...
          <div>
            <button
              type="submit"
              :disabled="!meta.valid || provider == 'google.com'"
              :class="
                !meta.valid || provider == 'google.com' ? 'opacity-30' : ''
              "
              class="flex w-full justify-center rounded-md px-3 p-1.5 text-sm font-semibold leading-6 text-ms-white bg-primary"
            >
              Nutzer anpassen
            </button>
          </div>
      ...
      <p class="text-sm text-gray-900">
        Erhalten Sie eine Email zum Zurücksetzen Ihres Passwortes.
      </p>
      <button
        @click="passwortReset"
        :disabled="provider == 'google.com'"
        class="flex w-full justify-center rounded-md px-3 p-1.5 text-sm leading-6 border-2 border-primary text-primary"
        :class="provider == 'google.com' ? 'opacity-30' : ''"
      >
      ...
      <p class="text-sm text-gray-900">Melden Sie sich von Kalenderio ab.</p>
      <a
        href="/logout"
        class="flex w-full justify-center rounded-md px-3 p-1.5 text-sm leading-6 border-2 border-primary text-primary"
      >
